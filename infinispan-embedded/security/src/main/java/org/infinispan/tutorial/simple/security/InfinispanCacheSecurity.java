...
public class InfinispanCacheSecurity {
    ...
    private static final Subject ADMIN_USER = createSubject(ADMIN_ROLE);
    private static final Subject SECRET_USER = createSubject(SECRET_ROLE);
    private static final Subject READ_ONLY_USER = createSubject(READ_ONLY_ROLE);
    private static final Subject WRITE_ONLY_USER = createSubject(WRITE_ONLY_ROLE);
...
//        Exception is not thrown when using like this.
//        Security.doAs(user, () -> {
//            c.forEach((key, value) -> System.out.println(key + " -> " + value));
//        });

        Security.doAs(user, () -> {
            c.entrySet().forEach(e -> System.out.println(e.getKey() + " -> " + e.getValue()));
        });
    }
